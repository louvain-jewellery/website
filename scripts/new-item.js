document.addEventListener('DOMContentLoaded', function() {
    ...
    function setThumbnailSizes() {
        ...
        let actualGapPx = 0;
        if (gapUnit === 'rem') {
            actualGapPx = gapValue * parseFloat(getComputedStyle(document.documentElement).fontSize);
        } else if (gapUnit === 'px') {
        ...
        imagesContainer.style.setProperty('--thumbnail-height', `${thumbnailHeight}px`);
    }

    setThumbnailSizes();
    window.addEventListener('resize', setThumbnailSizes);

    mainImageWrapper.querySelector('.main-image').addEventListener('load', setThumbnailSizes);
});